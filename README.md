...
כל הטקסטים באתר ניתנים לעריכה ישירות בדפדפן ללא צורך בקוד.

### שלבי עריכה

1. פתח את האתר בדפדפן.
2. לחץ על **✏️ עריכה** (כפתור זהב, תחתית-ימין).
3. לחץ על כל טקסט — הוא יהפוך לשדה עריכה (מסגרת זהובה).
4. ערוך, ואז לחץ **שמור** שמופיע מתחת לשדה.
5. לחץ **✓ סיום עריכה** לאשר את כל השינויים.

> **השינויים שמורים בדפדפן בלבד** (localStorage). הם לא מועלים לשרת.

### שמירה קבועה — מסירה למפתח

כדי שהשינויים יתקיימו לצמיתות גם לאחר פרסום:

1. לחץ **ייצוא תוכן** (כפתור כחול-זהב, ליד כפתור העריכה).
2. יורד קובץ `content.json`.
3. שלח את הקובץ למפתח — הוא יחליף את `lib/content.json` בפרויקט ויעלה גרסה חדשה.

### איפוס שינויים

לביטול כל השינויים ולחזרה לברירת המחדל:
פתח כלי מפתח (F12) ← Application ← Local Storage ← מחק את `eilat-adr-content`.
